...
const cancelExpiredReservationsAndMakeBooksAvailable = async () => {
    try {
        ...
        if (expiredReservations.length > 0) {
            try {
                const promises = expiredReservations.map(
                    async (reservation) => {
                        ...
                        if (book) {
                            ...
                            console.log(
                                `Livre ${book.BookID} marqué comme disponible.`
                            )
                        }

                        console.log(
                            `Réservation ${reservation.ReservationID} annulée, expirée.`
                        )
                    }
                )
                ...
                console.error(
                    'Erreur lors de la mise à jour des réservations:',
                    updateError
                )
        ...
        console.error(
            "Erreur lors de l'annulation des réservations expirées:",
            error
        )
...
const reducePenaltyPoints = async () => {
    try {
        ...
        // Retourner un succès
        console.log(
            'Points de pénalité réduits avec succès pour les utilisateurs en règle.'
        )
    } catch (error) {
        console.error(
            'Erreur lors de la réduction des points de pénalité',
            error
        )
    }
}
    ...
    try {
        ...
        for (const user of users) {
            ...
            sendEmailNotification(
                user.UserID,
                'Récompenses appliquées pour les utilisateurs réguliers.',
                'Vous avez reçu une récompense pour bonne régularité.'
            )
        }
        ...
        console.error(
            'Erreur lors de la récompense des utilisateurs réguliers',
            error
        )
    }
}
